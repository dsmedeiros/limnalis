"""Executable-documentation gate: run the marked Python snippets in the wiring docs.
...
attributes, wrong signatures, nonexistent paths). Every fixed snippet is now
marked for extraction, so any future drift between the docs and the API fails
the suite instead of silently misleading integrators.
...
# The wiring docs covered by this gate. Every ``runnable`` snippet in these
# files is executed; each file must contain at least one so the gate cannot be
# emptied by quietly deleting markers.
COVERED_DOCS = [
