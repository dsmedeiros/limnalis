...
class TestConformanceSessionBuilding:
    ...
    def test_build_sessions_from_environment_preserves_step_claim_subset(self):
        ...
        assert len(sessions) == 1
        step = sessions[0].steps[0]
        assert step.claim_subset == ["c1", "c3"]
...
class TestConformanceTransportQueries:
    ...
    def test_transport_query_step_index_accounts_for_implicit_default_steps(self):
        case = SimpleNamespace(
            environment={
                "sessions": [
                    {"id": "s1", "steps": []},
                        ...
                        "steps": [
                            {
                                "id": "step0",
                                "transport_queries": [
                                    {"id": "s2_step0_q", "bridgeId": "br1", "claimId": "c2"}
                                ],
                            }
                        ],
                    },
        ...
        assert queries[0]["id"] == "s2_step0_q"
