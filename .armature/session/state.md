...
## Active Delegation
M8 checkpoint 3 delegated (index/errata/deviations — final). Checkpoint 2 committed: 21 files, reviewer PASS_WITH_ADVISORIES with the aggregate_contested_adequacy code deviation execution-confirmed; suite 1119. Ledger additions for ckpt 3 filing: aggregate_contested_adequacy dual deviation (aggregation rules + B[method_conflict] trigger mechanisms), TransportNode.truthPolicy parsed-but-unread at runtime. Prior ckpt-2 handoff notes superseded: M8 checkpoint 2 delegated (contradictions/staleness per PRD items 5-11, PLUS checkpoint-1 handoff verified by reviewer: `format=` kwarg → `output_format=`/`input_format=` at 5 sites across 3 files (downstream_artifact_consumption.md:52, export_formats.md:192,193,222, exchange_package_format.md:216 — all raise TypeError, execution-confirmed); PrimitiveSet dual-module listing in plugin_sdk_overview.md:15,22; stale src comment src/limnalis/interop/envelopes.py:19-21 is REPORT-ONLY, no src changes). Checkpoint 1 committed: 9 docs + snippet gate, reviewer PASS_WITH_ADVISORIES, suite 1110.
